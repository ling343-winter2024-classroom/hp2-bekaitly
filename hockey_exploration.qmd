...
Association of height and birth month
```{r}
nhl_body_and_birth <- left_join(nhl_player_births, nhl_rosters, by = "player_id")
```

Association of weight and birth month
```{r}
ggplot(nhl_body_and_birth, aes(x = factor(birth_month), y = weight_in_pounds)) +
  geom_boxplot() +
  labs(x = "Birth Month", y = "Weight in pounds", title = "Height Distribution by Birth Month") 
```
